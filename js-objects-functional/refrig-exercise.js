...
class Food {
  ...
  class Refrigerator {
    ...
    putFood(food) {
        ...
        let searchedFoodIdx = -1;
        for(let i = 0; i < this.store.length; i++) {
            if(this.store[i].whatIsThis() == food.whatIsThis()) {
                searchedFoodIdx = i;
                break;
            }
        }
        ...
        if(searchedFoodIdx == -1) {
            // food does not yet exist in the refrigerator
            this.store.push(food);
        } else {
            // food does exist in the refrigerator
            this.store[searchedFoodIdx].qty += food.qty;
        }
    }
  
    getAndEatFood(foodName) {
        // Find the element from food store which matches the foodName
        let searchedFoodIdx = -1;
        for(let i = 0; i < this.store.length; i++) {
            if(this.store[i].whatIsThis() == foodName) {
                searchedFoodIdx = i;
                break;
            }
        }
